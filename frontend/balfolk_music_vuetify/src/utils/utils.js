...
export async function fetchEventData(event_type, objects, loading, countriesSelected) {
    loading.value = true;

    let countries = countriesSelected.join('&country=');
    var response = await axios.get("/events/?event_type=" + event_type + '&country=' + countries);
...
export const getCountriesToSelect = () => {
    return Object.keys(countryListAlpha2).map((key) => ({ alpha_2: key, name: countryListAlpha2[key] }));
}
